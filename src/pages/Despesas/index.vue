<template>
  <div class="pagina">
    <div class="text-h6 row justify-center"> Despesas do Usuario &nbsp; <span class="text-weight-bold text-blue-8"> {{
      usuarioLogado }}</span>
    </div>
    <div class="q-pa row q-gutter-md q-mx-xl cards">
      <q-card v-ripple @click="teste(item)" class="card-fixed-size cursor-pointer q-hoverable" v-for="item in despesas"
        :key="item" bordered>
        ...
        <div class="label-card">
          <q-card-section class="q-pt-none">
            <div class="text-h6"> <span>{{ item.name }}</span></div>
          </q-card-section>
        </div>
        <q-card-section>
...
export default defineComponent({
  ...
  components: {
    ...
    DespesasModal: defineAsyncComponent(() => import('./Modal/DespesasModal.vue'))
  },
  setup() {
    ...
    const despesas = [
      {
        ...
        description: 'SLKPALPSDAKÇALSKDjlkjsahdlkjs hdaskl djklasj daksljd klasj lkjqiowuewioqu askjdklsa SLKPALPSDAKÇALSKDjlkjsahdlkjs hdaskl djklasj daksljd klasj lkjqiowuewioqu askjdklsa SLKPALPSDAKÇALSKDjlkjsahdlkjs hdaskl djklasj daksljd klasj lkjqiowuewioqu askjdklsa'
      },
      {
        id: 2,
    ...
    const teste = (() => {
      Events.emit('open::modal::despesas', true)
      console.log('chamou');
    })
